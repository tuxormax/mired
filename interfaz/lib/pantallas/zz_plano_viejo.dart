...
class DatosMapaViejo {
  const DatosMapaViejo({
    required this.mapa,
...
const double anchoCajaV = 190;
const double altoCajaV = 54;
const double separacionXVV = 30;
const double separacionYVV = 130;
...
const double altoEnlacesVV = 90;

/// coloresParaExportarV son los que usa todo archivo exportado, sin importar si
...
/// sobre blanco.
final ColorScheme coloresParaExportarV =
    ColorScheme.fromSeed(seedColor: const Color(0xFF1565C0), brightness: Brightness.light);

/// _ArbolDeclaradoV ordena lo que se declaro a mano como lo que es: un arbol.
///
...
class _ArbolDeclaradoV {
  _ArbolDeclaradoV(this.datos) {
  ...
  double medir(Equipo equipo) {
    ...
    _anchos[equipo.id] = anchoCajaV + separacionXV; // provisional, corta circulos

    double total = 0;
    for (final puerto in datos.topologia.puertosDe(equipo.id)) {
      final hijo = hijoEn(equipo.id, puerto);
      total += hijo == null ? anchoCajaV + separacionXV : medir(hijo);
    }
    ...
    for (final cliente in _clientes[equipo.id] ?? const <EnlaceInalambrico>[]) {
      ...
      total += hijo != null && esCabecera(hijo.id)
          ? medir(hijo)
          : anchoCajaV + separacionXV;
    }

    final ancho = matematicas.max(total, anchoCajaV + separacionXV);
  ...
  double colocar({
    ...
    cajas.add(CajaPlanoV(
      rectangulo: Rect.fromCenter(center: centroEquipo, width: anchoCajaV, height: altoCajaV),
    ...
    for (final puerto in datos.topologia.puertosDe(equipo.id)) {
      final hijo = hijoEn(equipo.id, puerto);
      final cable = datos.topologia.enlaceDe(puerto.id);
      final anchoPuerto = hijo == null ? anchoCajaV + separacionXV : medir(hijo);
      final centroPuerto = Offset(xPuerto + anchoPuerto / 2, y + separacionYV);
      ...
      if (hijo != null) {
        ...
        cajas.add(CajaPlanoV(
          rectangulo: Rect.fromCenter(center: centroPuerto, width: anchoCajaV, height: altoCajaV),
      ...
      lineas.add(LineaPlanoV(
        desde: centroEquipo + const Offset(0, altoCajaV / 2),
        hasta: centroPuerto - const Offset(0, altoCajaV / 2),
        confirmada: false,
        declarada: true,
    ...
    for (final cliente in _clientes[equipo.id] ?? const <EnlaceInalambrico>[]) {
      ...
      final anchoCliente = esBloque ? medir(hijo) : anchoCajaV + separacionXV;
      final centroCliente = Offset(xPuerto + anchoCliente / 2, y + separacionYV);
      ...
      if (esBloque) {
        ...
        cajas.add(CajaPlanoV(
          rectangulo: Rect.fromCenter(center: centroCliente, width: anchoCajaV, height: altoCajaV),
      ...
      lineas.add(LineaPlanoV(
        desde: centroEquipo + const Offset(0, altoCajaV / 2),
        hasta: centroCliente - const Offset(0, altoCajaV / 2),
        confirmada: false,
        inalambrica: true,
    ...
    alBajar(y + separacionYV);
    return anchoBloque;
  }
}

/// armarPlanoViejo coloca todo: una columna por switch, y debajo de cada uno sus
...
PlanoV armarPlanoViejo(DatosMapaViejo datos, ColorScheme colores) {
  ...
  final cables = datos.mapa.enlacesUnicos
      ...
      .toList();
  final arriba = cables.isEmpty ? 0.0 : altoEnlacesV;

  double x = separacionXV;
  ...
  porSwitch.forEach((switchId, puertos) {
    ...
    final anchoBloque =
        matematicas.max(puertos.length * (anchoCajaV + separacionXV), anchoCajaV + separacionXV);

    final centroSwitch = Offset(x + anchoBloque / 2, arriba + separacionYV / 2);
    centrosDeSwitch[switchId] = centroSwitch;
    cajas.add(CajaPlanoV(
      rectangulo: Rect.fromCenter(
          center: centroSwitch, width: anchoCajaV, height: altoCajaV),
    ...
    puertos.forEach((indice, enElPuerto) {
      ...
      final centroPuerto = Offset(xPuerto + anchoCajaV / 2, arriba + separacionYV / 2 + separacionYV);

      if (confirmado) {
        final unico = enElPuerto.first;
        cajas.add(CajaPlanoV(
          rectangulo:
              Rect.fromCenter(center: centroPuerto, width: anchoCajaV, height: altoCajaV),
      ...
      lineas.add(LineaPlanoV(
        desde: centroSwitch + const Offset(0, altoCajaV / 2),
        hasta: centroPuerto - const Offset(0, altoCajaV / 2),
        confirmada: confirmado,
        etiqueta: enElPuerto.first.puerto,
      ));

      xPuerto += anchoCajaV + separacionXV;
    });

    x += anchoBloque + separacionXV * 2;
    anchoMaximo = matematicas.max(anchoMaximo, x);
    altoMaximo = matematicas.max(altoMaximo, arriba + separacionYV * 2 + altoCajaV);
  });
  ...
  for (final cable in cables) {
    ...
    enlaces.add(EnlacePlanoV(
      desde: desde - const Offset(0, altoCajaV / 2),
      hasta: hasta - const Offset(0, altoCajaV / 2),
      cima: Offset((desde.dx + hasta.dx) / 2, desde.dy - altoCajaV / 2 - altura),
  ...
  final arbol = _ArbolDeclaradoV(datos);
  if (arbol.raices.isNotEmpty) {
    if (altoMaximo == 0) altoMaximo = arriba + separacionYV / 2;
    double xManual = separacionXV;
    final yCabecera = altoMaximo + separacionYV / 2;
    ...
    for (final raiz in arbol.raices) {
      ...
      xManual += ancho + separacionXV * 2;
      anchoMaximo = matematicas.max(anchoMaximo, xManual);
    }

    altoMaximo = matematicas.max(altoMaximo, fondo + altoCajaV);
  }
  ...
  if (sinUbicar.isNotEmpty) {
    if (altoMaximo == 0) altoMaximo = arriba + separacionYV / 2;
    final yBase = altoMaximo + separacionYV;
    double xSuelto = separacionXV;
    double filaY = yBase;
    final porFila = matematicas.max(1, (anchoMaximo / (anchoCajaV + separacionXV)).floor());

    for (var i = 0; i < sinUbicar.length; i++) {
      final equipo = sinUbicar[i];
      if (i > 0 && i % porFila == 0) {
        xSuelto = separacionXV;
        filaY += altoCajaV + 20;
      }
      cajas.add(CajaPlanoV(
        rectangulo: Rect.fromLTWH(xSuelto, filaY, anchoCajaV, altoCajaV),
        titulo: equipo.comoSeLlama,
      ...
      xSuelto += anchoCajaV + separacionXV;
      altoMaximo = matematicas.max(altoMaximo, filaY + altoCajaV + separacionYV / 2);
    }
  }
  ...
  return PlanoV(
    ...
    lineas: lineas,
    enlaces: enlaces,
    tamano: Size(matematicas.max(anchoMaximo + separacionXV, 800),
        matematicas.max(altoMaximo + separacionYV, 600)),
...
class PintorMapaV extends CustomPainter {
  PintorMapaV({required this.plano, this.conFondo = false});
  ...
  void paint(Canvas lienzo, Size tamano) {
    ...
    for (final caja in plano.cajas) {
      ...
      _texto(lienzo, caja.titulo, caja.rectangulo.topLeft + const Offset(34, 8), 13,
          plano.colorTexto,
          negrita: true, ancho: anchoCajaV - 44);
